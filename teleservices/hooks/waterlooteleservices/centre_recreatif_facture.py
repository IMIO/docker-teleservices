...
# sudo -u wcs wcs-manage runscript --vhost=waterloo-formulaires.guichet-citoyen.be centre_recreatif_facture.py waterloo_facture
...
for formdef in FormDef.select(lambda x: x.id == "34", order_by="-receipt_time"):
    ...
    for formdata in formdef.data_class().select(
        lambda d: d.status != "draft",
        order_by="f1, f2, f151, f152, f179, f180, f207, f208, f292, f293, f264, f265, f236, f237",
    ):
        ...
        for field in formdef.get_all_fields():  # les autres champs
            if field.id in responsable:
                if hasattr(field, "get_display_value"):
                    str_responsable = "{} {}".format(
                        str_responsable, field.get_display_value(formdata.get_field_view_value(field))
                    )
                else:
                    str_responsable = "{} {}".format(
                        str_responsable, field.get_view_value(formdata.get_field_view_value(field))
                    )

            if field.id in adresse_responsable:
                if field.id == "19":
                    str_adresse_responsable = "{} (b{})".format(
                        str_adresse_responsable, field.get_view_value(formdata.get_field_view_value(field))
                    )
                else:
                    str_adresse_responsable = "{} {}".format(
                        str_adresse_responsable, field.get_view_value(formdata.get_field_view_value(field))
                    )

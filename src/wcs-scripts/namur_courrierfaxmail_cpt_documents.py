...
if __debug__:
    ...
    acte_divorce = {'doc': globals().get("form_var_tab_acte_divorce"), 'ex_places':(4,5)}
    acte_deces = {'doc': globals().get("form_var_tab_acte_deces"), 'ex_places':(4,5)}
    acte_mariage = {'doc': globals().get("form_var_tab_acte_mariage"), 'ex_places':(4,5)}
    acte_naissance = {'doc': globals().get("form_var_tab_acte_naissance"), 'ex_places':(3,4)}
    certif_cohab_legale = {'doc': globals().get("form_var_tab_cohab_legale"), 'ex_places':(3,4)}
    certif_nationalite = {'doc': globals().get("form_var_tab_nationalite"), 'ex_places':(3,4)}
    certif_residence = {'doc': globals().get("form_var_tab_residence"), 'ex_places':(3,4)}
    certif_residence_natio = {'doc': globals().get("form_var_tab_residence_nationalite"), 'ex_places':(3,4)}
    certif_residence_histo = {'doc': globals().get("form_var_tab_residence_histo"), 'ex_places':(3,4)}
    certif_vie = {'doc': globals().get("form_var_tab_certif_vie"), 'ex_places':(3,4)}
    compo_menage = {'doc': globals().get("form_var_tab_compo_menage"), 'ex_places':(3,4)}
    genealogie = {'doc': globals().get("form_var_tab_genealogie"), 'ex_places':(3,4)}
...
for key, tab_curr_document in documents.items():
    for row in tab_curr_document['doc']:
        ...
        retour = '{}{} copie(s) du formulaire : "{}" pour "{}" \r\n'.format(retour, nb_ex_doc, key, nom_prenom) 
result = retour
